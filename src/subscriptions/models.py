...
class Subscriptions(models.Model):
    ...
    features = models.TextField(
        null=True, blank=True, help_text="One line per feature")
    ...
    def get_features_as_list(self):
        if not self.features:
            return []
        return [x.string() for x in self.features.split("\n")]
...
class SubscriptionsPrice(models.Model):
    ...
    interval = models.CharField(
        max_length=120, choices=IntervalChoices.choices, default=IntervalChoices.MONTHLY)
    ...
    @property
    def display_featured_list(self):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.featured and self.subscription:
            qs = SubscriptionsPrice.objects.filter(
                subscription=self.subscription, interval=self.interval).exclude(id=self.id)
...
class UserSubscriptionQuerySet(models.QuerySet):
    ...
    def by_days_left(self, days_left=7):
        ...
        day_start = in_n_days.replace(
            hour=0, minute=0, second=0, microsecond=0)
        day_end = in_n_days.replace(
            hour=23, minute=59, second=59, microsecond=59)
        return self.filter(
...
def user_sub_post_save(sender, instance, *args, **kwargs):
    ...
    if ALLOW_CUSTOM_GROUPS:
        ...
        subs_qs = Subscriptions.objects.filter(
            avtive=True)
        if subscription_obj is not None:
            subs_qs = subs_qs.exclude(id=subscription_obj.id)
        subs_groups = subs_qs.values_list('id', flat=True)
...
post_save.connect(user_sub_post_save, sender=UserSubscriptions)
