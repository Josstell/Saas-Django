...
from subscriptions.models import Subscriptions, UserSubscriptions, SubscriptionStatus
...
def refresh_active_users_subscriptions(
        ...
        day_end=-1,
        verbose=False):
    qs = UserSubscriptions.objects.all()
    if active_only:
    ...
    if day_start > -1 and day_end > -1:
        qs = qs.by_range(days_start=day_start,
                         days_end=day_end, verbose=verbose)
    complete_count = 0
    qs_count = qs.count()
    for obj in qs:
        if verbose:
            print("Updating user", obj.user,
                  obj.subscription, obj.current_period_end)
        if obj.stripe_id:
...
def clear_dangling_subs():
    ...
    for customer_obj in qs:
        ...
        subs = helpers.billing.get_customer_active_subscriptions(
            customer_stripe_id)
        for sub in subs:
            existing_user_subs_qs = UserSubscriptions.objects.filter(
